    ...
    try:
        ...
        if 'changes' in cleaned_issue_data:
            if isinstance(cleaned_issue_data['changes'], list) and cleaned_issue_data['changes']:
                current_change = cleaned_issue_data['changes'][-1]
            del cleaned_issue_data['changes']
...
def list_excels():
    try:
        response = s3.list_objects_v2(Bucket=BUCKET_NAME,
                                      Prefix='sultan/escalation/excel/')
        files = []
        for obj in response.get('Contents', []):
...
def api_escalation_duplicate():
    ...
    try:
        ...
        # Save new escalation
        save_in_global_db(
            f'sultan/configs/draft/escalations/{new_escalation["id"]}.json',
            new_escalation)
